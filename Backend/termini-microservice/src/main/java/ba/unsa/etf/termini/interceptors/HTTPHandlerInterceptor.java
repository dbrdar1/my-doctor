package ba.unsa.etf.termini.interceptors;
...
public class HTTPHandlerInterceptor implements HandlerInterceptor {
    ...
    public void afterCompletion(HttpServletRequest request,
        ...
        if (request.getMethod().equals("GET")) {
            ...
            if (request.getServletPath().contains("/notifikacije-korisnika"))
                saveEventUsingGRPC("notifikacija", tipAkcijeGet, tipOdgovoraNaAkciju);
            if (request.getServletPath().contains("/notifikacije"))
                saveEventUsingGRPC("notifikacija", tipAkcijeGet, tipOdgovoraNaAkciju);
            if (request.getServletPath().contains("/termini-pacijenta"))
                saveEventUsingGRPC("termini", tipAkcijeGet, tipOdgovoraNaAkciju);
            if (request.getServletPath().contains("/termini-doktora"))
                saveEventUsingGRPC("termini", tipAkcijeGet, tipOdgovoraNaAkciju);
        }
        if (request.getMethod().equals("POST")) {
            ...
            if (request.getServletPath().contains("/dodaj-termin"))
                saveEventUsingGRPC("termini", tipAkcijeCreate, tipOdgovoraNaAkciju);
            if (request.getServletPath().contains("/dodaj-notifikaciju"))
                saveEventUsingGRPC("notifikacije", tipAkcijeCreate, tipOdgovoraNaAkciju);
            if (request.getServletPath().contains("/dodaj-vezu-pkd"))
                saveEventUsingGRPC("pacijentKartonDoktor", tipAkcijeCreate, tipOdgovoraNaAkciju);
        }
        if (request.getMethod().equals("DELETE")) {
            ...
            if (request.getServletPath().contains("/termini"))
                saveEventUsingGRPC("termini", tipAkcijeDelete, tipOdgovoraNaAkciju);
        }
        if (request.getMethod().equals("PUT")){
            ActionRequest.TipAkcije tipAkcijeUpdate = ActionRequest.TipAkcije.UPDATE;
            if (request.getServletPath().contains("/termini"))
                saveEventUsingGRPC("termini", tipAkcijeUpdate, tipOdgovoraNaAkciju);
        }
    }
    ...
    private void saveEventUsingGRPC(String resurs,
        ...
        String nazivMikroservisaTermini =
                "Termini";
        try {
            ...
            ActionResponse actionResponse = stub.registrujAkciju(ActionRequest.newBuilder()
                    ...
                    .setNazivMikroservisa(nazivMikroservisaTermini)
                    .setResurs(resurs)
