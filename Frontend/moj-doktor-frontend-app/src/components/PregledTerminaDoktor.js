...
const PregledTerminaDoktor = () => {
  ...
  useEffect(() => {
    ...
    const headers = new Headers({ "Authorization": 'Bearer ' + localStorage.getItem("token") });
    fetch('http://localhost:8080/termini/termini-doktora/' + localStorage.getItem("id"), { method: "get", headers })
      .then((res) => {
        return res.json();
      })
      .then((res) => {
        ...
        resTermini.forEach(termin => {
          const item = {
            key: termin.id,
            recipientId: termin.pacijent.id,
  ...
  function sendNotification(recipientId, datumTermina, vrijemeTermina) {
    ...
    const tekst = "Doktor " + localStorage.getItem("ime") + ' ' + localStorage.getItem("prezime") +
  ...
  return (
    <DoktorSideBar>
      <HeaderNaslovna stranica="Raspored termina" />
        ...
        <div className="tabela-termina">
          {loading ? <Loader /> :
            <>
              ...
              <div className="dugmad-termini">
                <button className="dugme-za-karticu" onClick={() => { history.push("/termin/dodaj") }}>Dodaj termin</button>
              </div>
            </>
          }
        </div>
...
export default PregledTerminaDoktor;